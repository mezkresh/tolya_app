...
private val DIFF_CALLBACK = object : DiffUtil.ItemCallback<StudentSubjectGroup>() {
    override fun areItemsTheSame(
        oldItem: StudentSubjectGroup,
        newItem: StudentSubjectGroup
    ): Boolean {
...
class StudentSubjectGroupAdapter(private val onClick: () -> Unit, var chosenDate: Date) :
    ListAdapter<StudentSubjectGroup, StudentSubjectGroupAdapter.ViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.buttonItemView.text =
            "${item.subjectName} ${item.teacherName} : ${item.startTime}-${item.endTime}"
        ...
        if (chosenDate.get(Calendar.DAY_OF_YEAR) == currentDate.get(Calendar.DAY_OF_YEAR)
            ...
            viewHolder.buttonItemView.setBackgroundColor(viewHolder.itemView.context.resources.getColor(R.color.green))
            viewHolder.buttonItemView.setOnClickListener {
                onClick()
            }
        }else{
            viewHolder.buttonItemView.setBackgroundColor(viewHolder.itemView.context.resources.getColor(
